...
import { SecretEchoJWTClaims } from "../models/token";
...
export class SecretEchoContext {
	...
	public Role: "admin" | "user" | "" = ""; // Add Role to context
...
export async function SecretEchoContextMiddleware(req: Request, res: Response, next: NextFunction) {
	...
	let decoded: SecretEchoJWTClaims;
	try {
		decoded = jwt.verify(token, getEnvConfig().TOKEN_SECRET) as SecretEchoJWTClaims;
	} catch (error: any) {
	...
	secretEchoCtx.Role = decoded.role; // Store the role in the context
